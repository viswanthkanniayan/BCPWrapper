...
public class OAuthTokenClient extends ApiClient {
    ...
    public String getAuthURL(JSONObject request) throws RazorpayException, JSONException {
    ...
    public OauthToken getAccessToken(JSONObject request) throws RazorpayException, JSONException {
        validateAccessTokenRequest(request);
        return post(null, Constants.TOKEN, request, Constants.AUTH);
    }

    public OauthToken refreshToken(JSONObject request) throws RazorpayException, JSONException {
        request.put(GRANT_TYPE, REFRESH_TOKEN);
        validateRefreshTokenRequest(request);
        return post(null, Constants.TOKEN, request, Constants.AUTH);
    }

    public OauthToken revokeToken(JSONObject request) throws RazorpayException, JSONException{
    ...
    private void validateAuthURLRequest(JSONObject request) throws RazorpayException, JSONException {
        payloadValidator.validate(request, getValidationsForAuthRequestURL());
    }

    private void validateAccessTokenRequest(JSONObject request) throws RazorpayException , JSONException{
        payloadValidator.validate(request, getValidationsForAccessTokenRequest());
    }

    private void validateRefreshTokenRequest(JSONObject request) throws RazorpayException , JSONException{
        payloadValidator.validate(request, getValidationsForRefreshTokenRequest());
    }

    private void validateRevokeTokenRequest(JSONObject request) throws RazorpayException , JSONException{
